...
import TousujianyiModel from '../models/TousujianyiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			let biaotimingcheng = req.query.biaotimingcheng
			if (biaotimingcheng) {

				if (biaotimingcheng.indexOf('%') != -1) {
					where.biaotimingcheng = {
						[Op.like]: biaotimingcheng
					}
				} else {
					where.biaotimingcheng = {
						[Op.eq]: biaotimingcheng
					}
				}
			}
			let xiangxineirong = req.query.xiangxineirong
			if (xiangxineirong) {

				if (xiangxineirong.indexOf('%') != -1) {
					where.xiangxineirong = {
						[Op.like]: xiangxineirong
					}
				} else {
					where.xiangxineirong = {
						[Op.eq]: xiangxineirong
					}
				}
			}
			let fankuileibie = req.query.fankuileibie
			if (fankuileibie) {

				if (fankuileibie.indexOf('%') != -1) {
					where.fankuileibie = {
						[Op.like]: fankuileibie
					}
				} else {
					where.fankuileibie = {
						[Op.eq]: fankuileibie
			...
			let jinjichengdu = req.query.jinjichengdu
			if (jinjichengdu) {

				if (jinjichengdu.indexOf('%') != -1) {
					where.jinjichengdu = {
						[Op.like]: jinjichengdu
					}
				} else {
					where.jinjichengdu = {
						[Op.eq]: jinjichengdu
			...
			let chulizhuangtai = req.query.chulizhuangtai
			if (chulizhuangtai) {

				if (chulizhuangtai.indexOf('%') != -1) {
					where.chulizhuangtai = {
						[Op.like]: chulizhuangtai
					}
				} else {
					where.chulizhuangtai = {
						[Op.eq]: chulizhuangtai
					}
				}
			}
			...
			let result = await TousujianyiModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await TousujianyiModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await TousujianyiModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await TousujianyiModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await TousujianyiModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await TousujianyiModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await TousujianyiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM tousujianyi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM tousujianyi WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM tousujianyi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM tousujianyi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM tousujianyi WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM tousujianyi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "tousujianyi"
			let where = " WHERE 1 = 1 "
			let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
			if(tableName == 'yezhu') {
				where += " AND yezhuzhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM tousujianyi " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("tousujianyi" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM tousujianyi " + where + " GROUP BY " + xColumnName + " DESC"
