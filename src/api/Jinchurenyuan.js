...
import JinchurenyuanModel from '../models/JinchurenyuanModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let jinrushijian = req.query.jinrushijian
			if (jinrushijian) {

				if (jinrushijian.indexOf('%') != -1) {
					where.jinrushijian = {
						[Op.like]: jinrushijian
					}
				} else {
					where.jinrushijian = {
						[Op.eq]: jinrushijian
			...
			let likaishijian = req.query.likaishijian
			if (likaishijian) {

				if (likaishijian.indexOf('%') != -1) {
					where.likaishijian = {
						[Op.like]: likaishijian
					}
				} else {
					where.likaishijian = {
						[Op.eq]: likaishijian
			...
			let renyuanxingming = req.query.renyuanxingming
			if (renyuanxingming) {

				if (renyuanxingming.indexOf('%') != -1) {
					where.renyuanxingming = {
						[Op.like]: renyuanxingming
					}
				} else {
					where.renyuanxingming = {
						[Op.eq]: renyuanxingming
					}
				}
			}
			let renyuanleixing = req.query.renyuanleixing
			if (renyuanleixing) {

				if (renyuanleixing.indexOf('%') != -1) {
					where.renyuanleixing = {
						[Op.like]: renyuanleixing
					}
				} else {
					where.renyuanleixing = {
						[Op.eq]: renyuanleixing
					}
				}
			}
			...
			let result = await JinchurenyuanModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await JinchurenyuanModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await JinchurenyuanModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await JinchurenyuanModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await JinchurenyuanModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await JinchurenyuanModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await JinchurenyuanModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM jinchurenyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM jinchurenyuan WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM jinchurenyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM jinchurenyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jinchurenyuan WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jinchurenyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "jinchurenyuan"
			let where = " WHERE 1 = 1 "
			let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
			if(tableName == 'wuyeguanliyuan') {
				where += " AND guanlizhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM jinchurenyuan " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("jinchurenyuan" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM jinchurenyuan " + where + " GROUP BY " + xColumnName + " DESC"
